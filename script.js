...
const handleYesClick = () => {
  question.innerHTML = "ETO GCASH KO!";
  ...
  letsGoBtn.addEventListener("click", () => {
      const randomIndex = Math.floor(Math.random() * Send_MO_Na.length);
      const selectedDateIdea = Send_MO_Na[randomIndex];

    alert(`Send mo na: ${selectedDateIdea}`);
  });
